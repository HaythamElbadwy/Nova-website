...
function ManagePlaylist() {
  ...
  useEffect(() => {
    const fetchPlaylists = async () => {
      try {
        const response = await fetch("https://nova-psi-five.vercel.app/playList/get", {
          method: 'GET',
          headers: {
  ...
  const activePlayeList = async (playListID) => {
    setIsDisabled(true)
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/playList/active/${playListID}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const handleDelete = async (playlistId) => {
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/playList/delete/${playlistId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const handlePasswordSubmit = async () => {
    setPasswordError('');
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/playList/checkPassword/${selectedPlaylist._id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
