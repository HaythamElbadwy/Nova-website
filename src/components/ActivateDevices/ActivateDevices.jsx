...
export default function ActivateDevices() {
  ...
  const handlePayment = async (e) => {
    ...
    // Payment Intent
    const response = await fetch('https://nova-psi-five.vercel.app/subscription/pay', {
      method: 'POST',
      headers: {
  ...
  const subscription = async () => {
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/subscription/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
